...
void band::calculateValues (int numSnapshots) {
  ...
  if (i>0) {
    ...
    */ 
    ...
    _tauavg = sqrt(_stressTensorAVG.row(1).norm()*_stressTensorAVG.row(1).norm() + _stressTensorAVG.row(2).norm()*_stressTensorAVG.row(2).norm());
    _pavg = _stressTensorAVG.row(0).norm();;
